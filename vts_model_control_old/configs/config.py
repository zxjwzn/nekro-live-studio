...
class ApiConfig(BaseModel):
    ...
    timeout: float = Field(
        default=5.0, description="超时时间，超过指定时间无API调用则恢复空闲动画"
    )
...
class PluginConfig(BaseModel):
    ...
    pre_animation_duration: float = Field(
        default=0.5, description="动画前置过渡时间（秒）"
    )
    expression_fade_time: float = Field(
        default=0.25, description="表情淡入淡出时间（秒），VTube Studio API默认0.5s"
    )
...
class BlinkConfig(BaseModel):
    ...
    min_interval: float = Field(
        default=2.0, description="两次眨眼之间的最小间隔时间（秒）"
    )
    max_interval: float = Field(
        default=4.0, description="两次眨眼之间的最大间隔时间（秒）"
    )
...
class BodySwingConfig(BaseModel):
    ...
    x_parameter: str = Field(
        default="FaceAngleX", description="身体左右摇摆控制的参数名"
    )
...
class MouthExpressionConfig(BaseModel):
    ...
    open_max: float = Field(
        default=0.7, description="嘴巴开合最大值（张开，可调小避免过度张嘴）"
    )
    change_min_duration: float = Field(
        default=2.0, description="表情变化最短持续时间（秒）"
    )
    change_max_duration: float = Field(
        default=7.0, description="表情变化最长持续时间（秒）"
    )
    smile_parameter: str = Field(
        default="MouthSmile", description="嘴角微笑控制的参数名"
    )
...
class SpeechSynthesisConfig(BaseModel):
    ...
    text_per_second_rate: float = Field(
        default=5.0, description="每秒播放的字数 (播放速率)"
    )
    ...
    volume: float = Field(
        default=0.5, ge=0.0, le=1.0, description="播放音量 (范围 0.0 到 1.0)"
    )
...
class BilibiliConfigs(BaseModel):
    ...
    ac_time_value: str = Field(
        default="", description="Bilibili直播ac_time_value 可不填"
    )
...
class VTSModelControlConfig(BaseModel):
    ...
    mouth_expression: MouthExpressionConfig = Field(
        default_factory=MouthExpressionConfig
    )
    speech_synthesis: SpeechSynthesisConfig = Field(
        default_factory=SpeechSynthesisConfig
    )
...
def load_config() -> VTSModelControlConfig:
    ...
    if CONFIG_FILE.exists():
        return VTSModelControlConfig.model_validate_json(

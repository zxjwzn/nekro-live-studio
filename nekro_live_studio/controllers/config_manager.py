...
class ConfigManager:
    ...
    async def load_config_for_current_model(self) -> None:
        ...
        try:
            with contextlib.suppress(Exception):
                ...
                if model_info and model_info.get("modelLoaded"):
                    ...
                    logger.info(f"检测到模型 '{self.current_model_name}' (ID: {model_info['modelID']})，将加载对应配置。")
                else:
        ...
        if self.current_model_name:
            ...
            logger.info("未加载任何模型，将使用空的内存配置，不进行保存。")
            self.config = ControllersConfig()
    ...
    async def on_model_loaded_event(self, event_data: Dict[str, Any]) -> None:
        ...
        if model_loaded:
            ...
            # 模型卸载
            if model_name:
                logger.info(f"检测到模型卸载事件：'{model_name}' (ID: {model_id}) 已卸载。")
            else:
                logger.info("检测到模型卸载事件。")

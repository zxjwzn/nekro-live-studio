...
class VTSClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.exception("连接到VTubeStudio失败")
            raise VTSConnectionError("连接到VTubeStudio失败")  # noqa: B904
    ...
    async def authenticate(self) -> bool:
        ...
        if not self.websocket or not self._connected:
            raise VTSConnectionError("未连接到VTubeStudio")
        ...
        try:
            ...
            is_authenticated = api_state_response.data.get("currentSessionAuthenticated", False)
            ...
            if auth_response.error:
                ...
                raise VTSAuthenticationError(f"认证失败: {error_msg}")  # noqa: TRY301
            ...
            if not authenticated:
                ...
                raise VTSAuthenticationError(f"认证失败: {reason}")  # noqa: TRY301
            ...
            logger.exception("认证过程中出现异常")
            raise VTSAuthenticationError("认证过程中出现异常")  # noqa: B904
    ...
    async def send_request_raw(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise VTSResponseError(f"请求超时: {request_data.get('messageType', 'Unknown')}") from e
        except Exception as e:
            self.pending_requests.pop(request_id, None)
            raise VTSResponseError(f"发送请求失败: {e!s}") from e
            ...
            while self._connected:
                try:
                    ...
                    try:
                        ...
                        # 检查是否是响应消息
                        if "requestID" in data and data["requestID"] in self.pending_requests:
                            ...
                            if not future.done():
                                # 检查是否有错误
                                if data.get("errorID"):
                                    error = VTSAPIError(data["errorID"])
                        ...
                        logger.exception(f"无法解析JSON响应: {message}")
                    except Exception:
                        logger.exception("处理消息时出错")

                except websockets.exceptions.ConnectionClosed:
                    logger.exception("WebSocket连接已关闭")
                    self._connected = False
                    break
                except Exception:
                    logger.exception("接收消息时出错")

        except asyncio.CancelledError:
            # 正常取消
            pass
        except Exception:
            logger.exception("接收消息循环出错")
        finally:
            # 确保所有挂起的请求都被取消
            for future in self.pending_requests.values():
                if not future.done():
                    future.set_exception(VTSConnectionError("连接已关闭"))
            self.pending_requests.clear()

    # --- 事件订阅方法 ---
    # --- 事件订阅方法 ---
    async def subscribe_to_event(self, event_name: str, config: Optional[Dict[str, Any]] = None) -> EventSubscriptionResponse:
        """
        订阅 VTube Studio 事件。
        ...
        if not self.is_authenticated:
            ...
            raise VTSAuthenticationError("需要先认证才能订阅事件")

        request = EventSubscriptionRequest(event_name=event_name, subscribe=True, config=config)
        ...
        if response.error:
            ...
            raise VTSAPIError(f"订阅事件失败: {error_msg}", error_id=response.error)
        ...
        if response.message_type == "EventSubscriptionResponse":
            ...
            logger.info(f"成功订阅事件: {event_name}。当前订阅数: {specific_response.subscribed_event_count}")
            return specific_response
        # 收到非预期的成功响应类型
        logger.error(f"订阅事件 {event_name} 收到非预期响应类型: {response.message_type}")
        raise VTSResponseError(f"订阅事件收到非预期响应类型: {response.message_type}")

    async def unsubscribe_from_event(self, event_name: Optional[str] = None) -> EventSubscriptionResponse:
        """
        ...
        if not self.is_authenticated:
            ...
            raise VTSAuthenticationError("需要先认证才能取消订阅事件")
        ...
        if response.error:
            error_msg = response.data.get("message", f"错误代码: {response.error}")
            logger.error(f"取消订阅事件 {event_name if event_name else '所有'} 失败 (API错误): {error_msg}")
            raise VTSAPIError(f"取消订阅事件失败: {error_msg}", error_id=response.error)
        ...
        if response.message_type == "EventSubscriptionResponse":
            ...
            return specific_response
        # 收到非预期的成功响应类型
        logger.error(f"取消订阅事件 {event_name if event_name else '所有'} 收到非预期响应类型: {response.message_type}")
        raise VTSResponseError(f"取消订阅事件收到非预期响应类型: {response.message_type}")
    ...
    def register_event_callback(
        ...
        if not event_type.endswith("Event"):
            logger.warning(f"注册的回调事件类型 '{event_type}' 可能不是有效的事件名称 (不以 'Event' 结尾)")
        ...
        if callback not in self.event_callbacks[event_type]:
            ...
            logger.warning(f"尝试重复注册事件 '{event_type}' 的回调函数: {callback.__name__}")
    ...
    def unregister_event_callback(
        ...
        if event_type in self.event_callbacks and callback in self.event_callbacks[event_type]:
            ...
            logger.info(f"已取消注册事件 '{event_type}' 的回调函数: {callback.__name__}")
            if not self.event_callbacks[event_type]:
                # 如果该事件类型没有回调了，可以考虑从字典中移除
                del self.event_callbacks[event_type]
        else:
            logger.warning(f"尝试取消注册不存在的回调函数或事件类型: {event_type} / {callback.__name__}")

...
class AudioManager:
    ...
    def __init__(self, rpg_sound_file_path: str, default_text_rate: float, default_volume: float):
        try:
            pygame.mixer.init()
            self.rpg_sound = pygame.mixer.Sound(rpg_sound_file_path)
            self.default_volume = max(0.0, min(1.0, default_volume)) # 确保音量在0.0-1.0之间
            self.rpg_sound.set_volume(self.default_volume)
            logger.info(f"RPG音效文件 {rpg_sound_file_path} 加载成功。默认音量设置为: {self.default_volume:.2f}")
        except pygame.error as e:
            logger.error(f"加载RPG音效文件 {rpg_sound_file_path} 失败: {e}")
            self.rpg_sound = None
        except Exception as e:
            logger.error(f"pygame.mixer 初始化或RPG音效加载失败: {e}. 部分音频功能可能不可用。")
            self.rpg_sound = None
        
        self.default_text_rate = default_text_rate # 存储配置中的默认文字播放速率
    ...
    async def play_text_as_speech(self, text_segments: List[str], speeds: Optional[List[float]] = None):
        ...
        for i, segment in enumerate(text_segments):
            ...
            if speeds and i < len(speeds) and speeds[i] > 0:
                ...
                logger.debug(f"RPG语音文本片段 {i+1} 使用自定义速率: {current_speed:.2f} 字/秒")
            else:
                logger.debug(f"RPG语音文本片段 {i+1} 使用默认速率: {current_speed:.2f} 字/秒 (原因: speeds列表未提供，或索引越界，或速度值无效)")
            
            char_interval = 1.0 / current_speed if current_speed > 0 else float('inf')

            logger.info(f"准备播放RPG语音文本片段 {i+1}/{len(text_segments)}: '{segment[:20]}...' ({num_chars}字), 速率: {current_speed:.2f} 字/秒, 音量: {self.rpg_sound.get_volume():.2f}")

            for char_index in range(num_chars):
                try:
                    self.rpg_sound.play()
                except pygame.error as e:
                    logger.error(f"播放RPG音效时出错 (片段 {i+1}, 字符 {char_index+1}): {e}")
    ...
    async def play_sound(self, audio_file_path: str, volume: Optional[float] = None, loops: int = 0):
        """
        播放指定路径的音频文件。
        ...
        try:
            ...
            logger.info(f"开始播放音频: {audio_file_path}, 音量: {play_volume:.2f}, 循环次数: {loops}")

            # 如果需要等待音频播放完成 (对于非循环或有限循环的情况)
            # pygame.Sound.play() 是非阻塞的。如果需要等待，可以这样做：
            if loops >= 0: # 对于无限循环，我们不等待
                # await asyncio.sleep(sound_to_play.get_length()) # get_length() 返回秒数
                # logger.info(f"音频 {audio_file_path} 播放完成。")
                pass # 通常我们不希望在这里阻塞，让调用者决定如何处理

...
class ConnectionManager:
    ...
    async def broadcast_to_path(self, path: str, message: str):
        ...
        if path in self.connections_by_path:
            await self._broadcast_to_connections(self.connections_by_path[path], message)

    async def _broadcast_to_connections(self, connections: List[WebSocket], message: str):
        """内部方法：向指定连接列表广播消息"""
        if not connections:
            return

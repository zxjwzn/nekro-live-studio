...
class AuthenticationRequest(VTSRequest):
    """使用认证令牌进行认证"""
    def __init__(self, plugin_name: str, plugin_developer: str, authentication_token: str, plugin_icon: Optional[str] = None):
        data = {
            "pluginName": plugin_name,
            "pluginDeveloper": plugin_developer,
...
class ExpressionActivationRequest(VTSRequest):
    ...
    def __init__(self, expression_file: str, active: bool = True, fade_time: float = 0.25):
        super().__init__(message_type="ExpressionActivationRequest", data={
...
class ModelLoadRequest(VTSRequest):
    ...
    def __init__(self, model_id: str):
        super().__init__(message_type="ModelLoadRequest", data={
            "modelID": model_id
        })
...
class MoveModelRequest(VTSRequest):
    ...
    size: 目标大小 (-100 到 100)
    """
    def __init__(self, time_in_seconds: float, values_are_relative_to_model: bool, positionX: Optional[float] = None, positionY: Optional[float] = None, rotation: Optional[float] = None, size: Optional[float] = None):
        data = {
...
class EventSubscriptionRequest(VTSRequest):
    ...
    """
    def __init__(self, event_name: Optional[str], subscribe: bool, config: Optional[Dict[str, Any]] = None):
        data: Dict[str, Any] = {
            "subscribe": subscribe
        }
        if event_name:

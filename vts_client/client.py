...
from .models import (
    VTSRequest, VTSResponse, 
    APIStateRequest, AuthenticationTokenRequest, AuthenticationRequest,
    EventSubscriptionRequest, EventSubscriptionResponse # 添加导入
)
...
class VTSClient:
    ...
    async def send_request_raw(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ResponseError(f"请求超时: {request_data.get('messageType', 'Unknown')}")
        except Exception as e:
            self.pending_requests.pop(request_id, None)
    ...
    async def unsubscribe_from_event(self, event_name: Optional[str] = None) -> EventSubscriptionResponse:
        ...
        if response.message_type == "EventSubscriptionResponse":
            ...
            # 更新本地订阅列表
            self.subscribed_events = specific_response.subscribed_events
            logger.info(f"成功取消订阅事件: {event_name if event_name else '所有事件'}。当前订阅数: {specific_response.subscribed_event_count}")
            return specific_response
        else:
            # 收到非预期的成功响应类型
            logger.error(f"取消订阅事件 {event_name if event_name else '所有'} 收到非预期响应类型: {response.message_type}")
            raise ResponseError(f"取消订阅事件收到非预期响应类型: {response.message_type}")

    # --- 事件回调注册 --- 
    def register_event_callback(self, event_type: str, callback: Callable[[Dict[str, Any]], Coroutine[Any, Any, None]]) -> None:
        """
    ...
    def unregister_event_callback(self, event_type: str, callback: Callable[[Dict[str, Any]], Coroutine[Any, Any, None]]) -> None:
        """
        取消注册事件回调函数。
